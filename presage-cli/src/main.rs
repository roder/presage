...
/// Look up a profile key for a UUID from contacts, profile_keys store, or group memberships.
async fn find_profile_key<S: Store>(
    manager: &Manager<S, Registered>,
    uuid: &Uuid,
) -> anyhow::Result<ProfileKey> {
    // Try contacts first
    if let Some(pk) = manager
        .store()
        .contact_by_id(uuid)
        .await?
